...
class ConfirmarHabitosFragment : Fragment() {
    ...
    companion object {
        ...
        fun newInstance(selectedHabits: ArrayList<String>, fechaInicio: Calendar?): ConfirmarHabitosFragment {
    ...
    override fun onCreateView(
        ...
        siButton.setOnClickListener {
            ...
            if (habitCount >= 2 && habitCount <= 4) {
                ...
                (requireActivity() as BienvenidaActivity).updateRegisteredHabits(selectedHabits ?: emptyList(), fechaInicio)

                // Verificar y desbloquear logros relacionados con los malos hábitos registrados
                verificarYDesbloquearLogros(habitCount)

                // Redirigir al menú principal
                requireActivity().supportFragmentManager.popBackStack(null, 1)
            } else {
                Toast.makeText(context, "Debes registrar entre 2 y 4 malos hábitos.", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun verificarYDesbloquearLogros(habitCount: Int) {
        val sharedPreferences = requireContext().getSharedPreferences("LogrosPrefs", Context.MODE_PRIVATE)
        val editor = sharedPreferences.edit()

        // Desbloquear el logro si el usuario registró al menos 2 hábitos
        if (habitCount >= 2) {
            ...
            if (logro != null && !logro.desbloqueado) {
                ...
                Toast.makeText(context, "¡Logro Desbloqueado: ${logro.titulo}!", Toast.LENGTH_SHORT).show()
            }
        }

        // Desbloquear el logro si el usuario registró 4 hábitos
        if (habitCount == 4) {
